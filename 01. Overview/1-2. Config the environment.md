...
AutoCar는 기본적으로 모니터나 키보드가 없기 때문에, PC를 이용해 원격으로 프로그램을 작성·실행해야 합니다. 이때 PC는 단순히 개발 환경을 제공할 뿐, 실제 프로그램 실행은 AutoCar 내부의 엣지 디바이스에서 이루어집니다.
...
**PC에서 AutoCar로 SSH 접속**

AutoCar에는 SSH 서버가 실행 중입니다. 따라서 PC에 SSH 클라이언트 역할을 수행할 툴(프리웨어인 putty 권장)을 설치한 후 AutoCar에 연결합니다. 
- putty 다운로드: https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html

AutoCar의 IP 주소를 이용해 성공적으로 연결되면 로그인 ID와 패스워드를 요구하는데, 다음과 같이 기본 계정이 사전에 설정되어 있습니다.
- ID : soda
- PW : soda

아래는 putty를 사용하여 장비에 접속하는 예시로, putty를 실행한 후 Session의 Host Name에 AutoCar의 IP 주소를 입력한 후 **Open** 버튼을 누르면 연결됩니다. 

![putty_init](res/putty_init.png)

![putty_shell_login](res/putty_shell_login.png)

로그인 단계에서 사전 계정 정보를 입력하면, AutoCar 쉘이 실행됩니다.

![connect_complete](res/connect_complete.png)
...
다음 명령으로 주변 Wi-Fi 공유기 목록을 확인합니다. 
...
1. 다음 URL에서 VSCode를 다운받은 후 PC에 설치합니다. 

    https://code.visualstudio.com/

2. Download 링크를 누른 후 PC 환경에 맞는 설치 파일을 다운로드합니다.

![vscode_download](res/vscode_download.png)

**VSCode 확장 설치**

VSCode를 AutoCar 개발환경으로 활용하려면 다음과 같은 VSCode 확장이 필요합니다.  
- Python (Microsoft)
- Remote - SSH (Microsoft)

VSCode를 실행한 후 왼쪽 사이드바에서 Extensions를 선택한 후 검색 창에 이름을 입력하면 설치 가능한 목록이 표시되는데, 반드시 게시자가 Microsoft인 항목을 설치하세요.

**AutoCar 원격 접속 설정**

`Remote - SSH` 확장이 설치되면 사이드 바에 `Remote Explorer`가 추가됩니다. 이 곳에 AutoCar 원격 접속 정보를 한 번만 등록하면 이를 바탕으로 AutoCar에 접속할 수 있어 편리합니다. 등록 과정에서 \<유/무선 IP\>는 유선 연결 후 사용하는 192.168.101.101 또는 무선 연결 후 확인된 IP(예: 192.168.0.50) 중 하나를 입력하면 됩니다.

1. Command Palette(\<Ctrl>\<Shift>p)를 띄우고 ‘Remote-SSH: Add New SSH Host...’입력 
2. 연결 입력창에 ID@IP 형식으로 입력, ex) soda@192.168.101.101 또는 soda@192.168.6.50 등
...
2. 목록에서 \<유/무선 IP> 항목을 마우스 오른쪽 버튼으로 클릭하고 Connect to Host in Current Window를 선택합니다.
3. “Select the platform of the remote host \<유/무선 IP>” 메시지가 나타나면 Linux를 선택합니다.
4. Windows 보안 경고 창이 나타나면 액세스 허용을 선택합니다.
5. “Enter password for soda@<유/무선 IP>” 메시지가 표시되면 soda를 입력합니다.
...
11. ‘Python’을 검색하고, **Install in SSH: <유/무선 IP>**를 선택하여 설치합니다.
