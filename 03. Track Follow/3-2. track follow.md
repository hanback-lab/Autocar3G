...
실습을 진행하기에 AutoCar를 실습시킬 트랙을 구성합니다. 참고로 트랙 주행은 벽, 차선, 콘 등을 이용해 트랙을 만들고 차량이 주행할 때 발생하는 여러 상황을 수집하여 학습시킬 수 있습니다. 

트랙은 콘 < 벽 < 차선 순으로 차선이 가장 높은 인식률을 보입니다. 차선을 그릴 때는 바닥색과 대비되는 색의 종이 테이프를 사용하면 좋습니다. 본 교재는 차선 인식을 기준으로 실습을 진행합니다.
...
<details>
<summary><b>키보드 제어 프로그램</b></summary>
...
<details>
<summary><b>영상 녹화 프로그램</b></summary>
...
아래 코드를 사용하여 위 데이터 수집 과정에서 녹화한 영상을 프레임 단위로 추출하여 데이터를 저장합니다. 데이터 저장 위치는 'track_dataset' 이며, 바로 다음 과정에서 사용됩니다. 폴더 이름을 변경하지 말아주시길 바랍니다. 
...
본 교재에서는 제공되는 툴을 사용해 라벨링을 진행합니다. 아래 과정을 통해 데이터 라벨링 툴을 설치 및 실행시킵니다.

1. 명령 프롬프트 및 터미널에 다음 명령어를 입력하여 필수 패키지를 설치합니다.
...
![annotator_labeling](res/annotator_labeling.png)
...
위 과정에서 라벨링한 데이터들을 바탕으로 학습을 진행해보겠습니다. 학습 환경은 Colab, 사용자 PC 등 원하는 환경에서 진행합니다. 본 교재에서는 Colab을 기반으로 진행하겠습니다.
