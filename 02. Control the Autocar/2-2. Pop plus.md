...
AutoCar용 `Pop plus` 라이브러리는 PyPI에 등록되어 있으며, 아래와 같이 pip 명령을 통해 설치할 수 있습니다. 아래 명령어 실행 시 Pop plus 라이브러리 뿐만 아닌 실습에 필요한 라이브러리까지 모두 자동으로 설치됩니다. 다만 AutoCar 장비에는 해당 라이브러리가 기본적으로 사전 설치되어 있으므로, 별도의 설치 과정 없이 바로 사용할 수 있습니다.
...
프로그래밍을 시작하기 전에 작업 폴더에 `product` 파일을 생성해야 합니다. 이 파일은 `Pop plus` 라이브러리가 AutoCar 장비와 통신하기 위해 필요한 환경 정보를 정의하며, 각 항목의 의미는 다음과 같습니다.

- BROKER_DOMAIN
  - 통신 중계 역할을 수행하는 MQTT 브로커의 주소입니다.
  - Edge 디바이스에 SSH로 직접 접속한 상태에서는 127.0.0.1을 사용합니다.
  - SSH 없이 외부 PC에서 제어하는 경우에는 AutoCar 장비의 무선 IP 주소를 입력합니다.
- CAMERA_DOMAIN
  - 실시간 카메라 스트리밍을 제공하는 영상 서버의 주소입니다.
  - 장비의 무선 IP를 고정 값으로 작성합니다.
- DEVICE_NAME
  - 장비 이름을 지정하는 항목입니다. AutoCar G의 경우 autocar3g로 고정합니다.
- DEV_NUM
  - 장비의 고유 번호입니다. 동일 네트워크에 여러 대의 AutoCar가 연결된 경우 장비를 개별적으로 식별하기 위해 사용합니다.
- INSITUTION_NAME
  - 장비를 사용하는 기관(학교, 연구실 등)의 이름을 입력합니다.

장비의 고유 정보는 출고 시 기본적으로 설정되어 있으며, 아래 명령어를 통해 해당 내용을 확인할 수 있습니다. 이 중 BROKER_DOMAIN은 환경에 따라 달라질 수 있으므로 상황에 맞게 설정하고, 나머지 항목들은 명령어 출력값과 동일하게 작성해주시기 바랍니다.
...
위 설명을 참고하여 아래와 같은 형식으로 내용을 작성한 뒤, 파일명을 `product` 로 저장합니다. 상술하였듯, BROKER_DOMAIN 및 CAMERA_DOMAIN은 프로그램 실행 위치 (SSH 접속 유무)에 따라서 반드시 Domain이 변경되어져야만 합니다.

```
BROKER_DOMAIN=192.168.0.50
CAMERA_DOMAIN=192.168.0.50
